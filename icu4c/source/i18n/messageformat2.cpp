...
[[nodiscard]] InternalValue MessageFormatter::apply(const FunctionName& functionName,
    ...
    if (isFunction(functionName)) {
        LocalPointer<Function> functionImpl(getFunction(functionName, status));
        if (U_FAILURE(status)) {
        ...
        U_ASSERT(functionImpl != nullptr);

        FunctionValue val = (*functionImpl)(std::move(argument), std::move(options), status);
        ...
        return InternalValue(std::move(val));
    }
    return InternalValue(fallback);
}
#endif
        ...
        if (part.isText()) {
	      ...
	      InternalValue partVal = formatExpression(globalEnv, part.contents(), context, status);
              if (partVal.isFallback()) {
                  ...
                  result += partVal.takeValue(status)->formatToString(status);
